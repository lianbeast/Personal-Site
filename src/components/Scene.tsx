  ...
  return (
    <Html position={[0, -4.35, 0]} center zIndexRange={[30, 0]} style={{ pointerEvents: 'auto', textAlign: 'center' }}>
      <div className="flex flex-col items-center gap-1 sm:gap-2 select-none">
        <h1 className={`font-display font-black tracking-[0.2em] text-white holo-glow ${mobile ? 'text-xl' : 'text-3xl'}`}>{site.name}</h1>
        <p className={`${mobile ? 'text-[9px] tracking-[0.2em]' : 'text-[11px] tracking-[0.3em]'} text-cyan-200/80 uppercase`}>{site.tagline}</p>
        <div className="mt-1 sm:mt-2 flex gap-2 sm:gap-3">
          {chips.map(({ href, label, Icon }) => (
            <a
              ...
              aria-label={label}
              title={label}
              className={`rounded-full border border-cyan-400/30 bg-slate-950/70 text-cyan-300 transition hover:border-cyan-300 hover:text-white hover:shadow-[0_0_15px_rgba(34,211,238,0.4)] ${mobile ? 'p-1.5' : 'p-2'}`}
            >
              <Icon />
            </a>
          ))}
...
export function Scene({ paused, onFocusWeather, onFocusNews }: SceneProps) {
  const mobile = useIsMobile()
  // Tighter orbits on mobile so cards don't fly off-screen.
  const r = mobile ? 0.7 : 1  // scale factor
  return (
    <Canvas camera={{ position: [0, 0, 11], fov: 50 }} dpr={[1, 1.75]} gl={{ antialias: true }}>
      <color attach="background" args={['#020617']} />
      <fog attach="fog" args={['#020617', 18, 42]} />
      <Stars radius={60} depth={40} count={mobile ? 400 : 900} factor={4} fade speed={0.6} />

      <Globe />
      <SocialOrbit />
      <Identity />

      <OrbitingCard paused={paused} radius={6.4 * r} height={2.2 * r} speed={0.12} phase={0.6}>
        <WeatherCard onFocus={onFocusWeather} />
      </OrbitingCard>

      <OrbitingCard paused={paused} radius={6.9 * r} height={0.3 * r} speed={-0.1} phase={2.4}>
        <NewsCard kind="world" icon="📰" title="World News" onFocus={(a) => onFocusNews('world', a)} />
      </OrbitingCard>

      <OrbitingCard paused={paused} radius={6.1 * r} height={-2.0 * r} speed={0.14} phase={4.2}>
        <NewsCard kind="tech" icon="⚡" title="Tech News" onFocus={(a) => onFocusNews('tech', a)} />
      </OrbitingCard>

      <OrbitingCard paused={paused} radius={6.7 * r} height={-0.5 * r} speed={-0.08} phase={5.8}>
        <MapCard />
      </OrbitingCard>
    </Canvas>
  )
